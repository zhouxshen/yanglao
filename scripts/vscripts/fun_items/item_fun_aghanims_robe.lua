...
function IsReady(keys)
	...
	if caster:HasModifier("modifier_item_fun_Aghanims_Robe_min_health") then	    
	    if not ShouldHaveMinHealth(keys) then
		    caster:RemoveModifierByName("modifier_item_fun_Aghanims_Robe_min_health")
		end
	else
	    if ShouldHaveMinHealth(keys) then
		    ability:ApplyDataDrivenModifier(caster, caster, "modifier_item_fun_Aghanims_Robe_min_health", nil)
		end
	end
...
function Damage_Immune(keys)
    ...
    local caster = keys.caster
	local ability = keys.ability
	local cooldown = ability:GetEffectiveCooldown(1)--GetSpecialValueFor("immune_cooldown")
									      ...
									      or caster:HasModifier("modifier_troll_warlord_battle_trance") 
										     or caster:HasModifier("modifier_templar_assassin_refraction_absorb") 
											    or caster:HasModifier("modifier_monkey_king_transform") 
												   or caster:HasModifier("modifier_item_fun_Aghanims_Robe_damage_immune")
