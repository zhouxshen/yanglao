...
function modifier_item_assault_datadriven_enemy_aura_on_interval_think(keys)
	...
	if is_emitter_visible and not keys.target:HasModifier("modifier_item_assault_v2_datadriven_enemy_aura_visible") then
		keys.ability:ApplyDataDrivenModifier(keys.caster, keys.target, "modifier_item_assault_v2_datadriven_enemy_aura_visible", {duration = -1})
	elseif not is_emitter_visible and keys.target:HasModifier("modifier_item_assault_v2_datadriven_enemy_aura_visible") then
		keys.target:RemoveModifierByNameAndCaster("modifier_item_assault_v2_datadriven_enemy_aura_visible", keys.caster)
	end
end
